"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FeedController = void 0;
const openapi = require("@nestjs/swagger");
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const feed_service_1 = require("../services/feed.service");
const FollowersFollowing_dto_1 = require("../dto/FollowersFollowing.dto");
const PersonalizedParties_dto_1 = require("../dto/PersonalizedParties.dto");
const Search_dto_1 = require("../dto/Search.dto");
let FeedController = class FeedController {
    constructor(feedService) {
        this.feedService = feedService;
    }
    async search(searchDto, request) {
        return await this.feedService.search(searchDto, request.raw.decoded.id);
    }
    async getPersonalizedParties(personalizedParties, request) {
        return await this.feedService.getPersonalizedParties(personalizedParties, request.raw.decoded.id);
    }
    async getFollowers(followerDto) {
        return await this.feedService.getFollowers(followerDto);
    }
    async getFollowing(followingDto) {
        return await this.feedService.getFollowing(followingDto);
    }
};
exports.FeedController = FeedController;
__decorate([
    (0, common_1.Get)("search"),
    (0, common_1.UsePipes)(new common_1.ValidationPipe({
        transform: true,
        forbidNonWhitelisted: true,
        disableErrorMessages: false
    })),
    openapi.ApiResponse({ status: 200 }),
    __param(0, (0, common_1.Query)()),
    __param(1, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Search_dto_1.SearchDto, Object]),
    __metadata("design:returntype", Promise)
], FeedController.prototype, "search", null);
__decorate([
    (0, common_1.Get)("personalized-parties"),
    (0, common_1.UsePipes)(new common_1.ValidationPipe({
        transform: true,
        forbidNonWhitelisted: true,
        disableErrorMessages: false
    })),
    openapi.ApiResponse({ status: 200 }),
    __param(0, (0, common_1.Query)()),
    __param(1, (0, common_1.Req)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [PersonalizedParties_dto_1.PersonalizedPartiesDto, Object]),
    __metadata("design:returntype", Promise)
], FeedController.prototype, "getPersonalizedParties", null);
__decorate([
    (0, common_1.Get)("followers"),
    (0, common_1.UsePipes)(new common_1.ValidationPipe({
        transform: true,
        forbidNonWhitelisted: true,
        disableErrorMessages: false
    })),
    openapi.ApiResponse({ status: 200 }),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [FollowersFollowing_dto_1.FollowersFollowingDto]),
    __metadata("design:returntype", Promise)
], FeedController.prototype, "getFollowers", null);
__decorate([
    (0, common_1.Get)("following"),
    (0, common_1.UsePipes)(new common_1.ValidationPipe({
        transform: true,
        forbidNonWhitelisted: true,
        disableErrorMessages: false
    })),
    openapi.ApiResponse({ status: 200 }),
    __param(0, (0, common_1.Query)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [FollowersFollowing_dto_1.FollowersFollowingDto]),
    __metadata("design:returntype", Promise)
], FeedController.prototype, "getFollowing", null);
exports.FeedController = FeedController = __decorate([
    (0, swagger_1.ApiTags)("Feed"),
    (0, swagger_1.ApiBearerAuth)(),
    (0, common_1.Controller)("feed"),
    __metadata("design:paramtypes", [feed_service_1.FeedService])
], FeedController);
//# sourceMappingURL=feed.controller.js.map